...
<ul class="thumbnailCategories">
  ...
  {assign var=derivative value=$pwg->derivative($stripped_responsive.albumSize, $cat.representative.src_image)}
  {if !$derivative->is_cached()}
  {combine_script id='jquery.ajaxmanager' path='themes/default/js/plugins/jquery.ajaxmanager.js' load='footer'}
  {combine_script id='thumbnails.loader' path='themes/default/js/thumbnails.loader.js' require='jquery.ajaxmanager' load='footer'}
  {/if}
  <li class="odd{if ($smarty.foreach.catloop.iteration-1) % 2 == 0}0{else}1{/if}" {if ($stripped_responsive.albumFrame)}onclick="window.location='{$cat.URL}';"{/if}>
	<a href="{$cat.URL}">
