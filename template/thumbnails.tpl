...
<ul class="thumbList">
  ...
  {assign var=derivative value=$pwg->derivative($stripped_responsive.thumbSize, $thumbnail.src_image)}
  ...
  {/if}
		<li>
			<span class="wrap1">
				<span class="wrap2 {if !($stripped_responsive.showThumbLegend)}wrap2full{/if}">
					<a href="{$thumbnail.URL}"><div class="thumbframe"><div class="thumbpos">
						...
						{if (isset($stripped_responsive.imagePreload) & isset($stripped_responsive.imagePreloadThumbs))}
							{assign var=preload value=$pwg->derivative($stripped_responsive.imageSize, $thumbnail.src_image)}
							<div class="preload" style="display:none;">{$preload->get_url()}</div>
						{/if}
					</div></div></a>
				</span>
				<span class="thumbLegend {if !($stripped_responsive.showThumbLegend)}noLegend{/if}">
